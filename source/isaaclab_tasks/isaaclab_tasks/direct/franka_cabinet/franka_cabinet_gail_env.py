...
@configclass
class FrankaCabinetEnvGAILCfg(DirectRLEnvCfg):
    # env
    ...
    # motion loader
    MOTIONS_PATH = "logs/skrl/franka_cabinet_direct/memories/25-05-27_18-34-08-145742_memory_0x7f1cf1124850.npz"
    num_amp_observations = 1



class FrankaCabinetEnvGAIL(DirectRLEnv):
    ...
    cfg: FrankaCabinetEnvGAILCfg

    def __init__(self, cfg: FrankaCabinetEnvGAILCfg, render_mode: str | None = None, **kwargs):
        ...
        self.amp_observation_buffer = torch.zeros(
            (self.num_envs, self.cfg.num_amp_observations, self.cfg.observation_space), device=self.device
        )

        self.amp_action_size = self.cfg.num_amp_observations * self.cfg.action_space
        self.amp_action_space = gym.spaces.Box(low=-np.inf, high=np.inf, shape=(self.amp_action_size,))
        self.amp_action_buffer = torch.zeros(
            (self.num_envs, self.cfg.num_amp_observations, self.cfg.action_space), device=self.device
        )
